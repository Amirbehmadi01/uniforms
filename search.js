...
// // // main item headers
// // const keywords = ["پیراهن","شلوار","کت و شلوار","مانتو","کفش","کاپشن","جلیقه کت و شلوار","مقنعه"];
...
// // // parse shamsi string 'YYYY/MM/DD' or 'YYYY-MM-DD' to JS Date (gregorian)
// // function parseShamsiToDate(sh){
// //     if(!sh) return null;
// //     const s = String(sh).trim().replace(/-/g,'/'); // normalize
// //     const parts = s.split('/');
...
// // // convert JS Date to Shamsi string YYYY/MM/DD
...
// //         filteredData = xlsx.utils.sheet_to_json(sheet,{defval:""}).map(r => ({
...
// //         const sheet = wb.Sheets[wb.SheetNames[0]];
// //         bottleData = xlsx.utils.sheet_to_json(sheet,{defval:""}).map(r => ({
...
// //     // build all records from filteredData and bottleData (ensure no duplicates)
// //     const allRecords = [...filteredData.map(r => Object.assign({}, r))];

// //     bottleData.forEach(b=>{
// //         if(!allRecords.some(r=>r.code===b.code && r.name===b.name))
// //             allRecords.push({code:b.code, name:b.name, item:null, date:b.date || null});
// //     });

// //     const filtered = allRecords.filter(r => (r.code && r.code.includes(q)) || (r.name && r.name.includes(q)));
...
// //     // group by code (if code empty, group by name)
// //     const grouped = {};
// //     filtered.forEach(r=>{
...
// //     // build table header
// //     let table = '<table border="1" style="border-collapse:collapse;width:100%;background:white;"><thead>' +
// //         '<tr><th>نام</th><th>کد پرسنلی</th>';
// //     keywords.forEach(k=> table += `<th>${k}</th>`);
// //     table += '<th>قمقمه</th><th>به‌روزرسانی</th></tr></thead><tbody>';

// //     const today = new Date();

// //     for(const key in grouped){
// //         const rows = grouped[key];
// //         const name = (rows.find(r=>r.name) && rows.find(r=>r.name).name) || "";
// //         const code = (rows.find(r=>r.code) && rows.find(r=>r.code).code) || "";
...
// //                     let type = r.item.includes('استوک') ? 'استوک' : 'نو';
// //                     lastText[k] = (type === 'استوک' ? 'استوک - ' : '') + r.date;
// //                     lastDate[k] = r.date;
// //                 }
// //             });
// //         });

// //         const relatedBottle = bottleData.filter(b => (b.code && b.code === code) || (b.name && b.name === name));
// //         let bottleText = "-";
// //         relatedBottle.forEach(b => { if(b.date) bottleText = b.date; });
...
// //                     if(diffDays > 365) style = 'background:rgba(144,238,144,0.5)'; // light green
// //                     else style = 'background:rgba(240,128,128,0.35)'; // light red
// //                 }
// //             }
// //             row += `<td style="${style}">${escapeHtml(txt)}</td>`;
// //         });
// //         row += `<td>${escapeHtml(bottleText)}</td>`;
...
// //         '<label>آیتم:</label><br>' +
// //         '<select id="updateField" style="width:90%;padding:6px;margin:6px 0"><option value="">-- انتخاب آیتم --</option>' +
// //         modalOptions +
// //         '</select><br>' +
...
// main item headers  ←  قمقمه به این لیست اضافه شد
