...
//         '<!doctype html><html lang="fa"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
//         '<title>جستجو</title>' +
//         '<style>body{font-family:sans-serif;direction:rtl;background:#f5f5f5;display:flex;justify-content:center;align-items:center;height:100vh}</style>' +
//         '</head><body>' +
//         '<form method="POST" action="/search"><h2>جستجوی کالا</h2>' +
//         '<input name="query" placeholder="نام یا کد" required />' +
//         '<button type="submit">جستجو</button>' +
//         '</form></body></html>'
//     );
// });
...
//     if(!filtered.length){
//         return res.send(
//             '<!doctype html><html lang="fa"><head><meta charset="utf-8"></head><body>' +
//             `<h3>هیچ رکوردی یافت نشد برای: ${escapeHtml(q)}</h3><a href="/">بازگشت</a></body></html>`
//         );
//     }
...
//     let table = '<table border="1" style="border-collapse:collapse;width:100%;background:white;">' +
//         '<thead><tr><th>نام</th><th>کد</th>';

//     keywords.forEach(k=> table+=`<th>${k}</th>`);
//     table += '<th>آپدیت</th></tr></thead><tbody>';
...
//                     if(diff>365) style='background:#cfc;';
//                     else style='background:#fbb;';
//                 }
//             }
//             row+=`<td style="${style}">${escapeHtml(lastText[k])}</td>`;
//         });
...
// #box{background:white;padding:20px;width:380px;border-radius:8px;text-align:center}
// #downloadBtn{position:absolute;top:20px;right:20px;padding:10px 18px;font-size:16px;cursor:pointer}
// </style>
// </head><body>

// <button id="downloadBtn" onclick="downloadExcel()">دانلود با فرمت اکسل</button>
...
//     <h3>به‌روزرسانی</h3>
//     <label>نام</label><br>
//     <input id="updateName" readonly style="width:90%;padding:6px;margin:6px 0"><br>
//     <label>آیتم:</label><br>
//     <select id="updateField" style="width:80%;padding:6px;margin:6px 0">
//         <option value="">-- انتخاب --</option>
//         ${modalOptions}
//     </select><br>
//     <label>وضعیت:</label><br>
//     <select id="updateType" style="width:90%;padding:6px;margin:6px 0">
...
//     <button onclick="submitUpdate()">ذخیره</button>
//     <button onclick="closeModal()">لغو</button>
// </div></div>

// <script>
// let currentCode = null;
// function openUpdate(code,name){
//     currentCode = code;
...
//     if(!field || !date){ alert("پر کنید"); return; }
//     date=date.replace(/-/g,"/");
//     let itemText = type==="استوک" ? ("استوک "+field) : field;
//     fetch("/update",{method:"POST",headers:{"Content-Type":"application/json"},
//         body:JSON.stringify({code:currentCode,name,item:itemText,date})})
...
//         body:JSON.stringify({ query: "${escapeHtml(q)}" })
//     })
//     .then(r=>{
//         if(r.status===204){ alert("فایل خالی می‌باشد"); return null; }
//         return r.blob();
//     })
//     .then(blob=>{
//         if(!blob) return;
...
//         a.href=url; a.download="SearchResults.xlsx"; a.click();
...
//         return res.send('با موفقیت ذخیره شد');
//     }catch(e){
//         return res.status(500).send('خطا: '+e.message);
//     }
// });
...
//     if (!filtered.length) return res.status(204).send();
...
//     res.setHeader("Content-Disposition", "attachment; filename=SearchResults.xlsx");
...
// ================= CONFIG =================
const keywords = [
  "پیراهن","شلوار","کت و شلوار","مانتو",
  "کفش","کاپشن","جلیقه کت و شلوار","مقنعه","قمقمه"
];
...
function loadFilteredData(){
  try{
    ...
    filteredData = xlsx.utils.sheet_to_json(sh,{defval:""}).map(r=>({
      code: clean(r["کد پرسنلی"]||r["A"]),
      name: clean(r["نام کارمند"]||r["E"]),
      item: clean(r["نام کالا"]||r["C"]||""),
      date: String(r["تاریخ تحویل"]||r["D"]||"").replace(/-/g,"/")
...
function loadBottleData(){
  try{
    const wb = xlsx.readFile(bottlePath);
    const sh = wb.Sheets[wb.SheetNames[0]];
    bottleData = xlsx.utils.sheet_to_json(sh,{defval:""}).map(r=>({
      code: clean(r["A"]||r["کد پرسنلی"]),
      name: clean(r["E"]||r["نام کارمند"]),
      date: String(r["D"]||r["تاریخ تحویل"]||"").replace(/-/g,"/")
    }));
...
<h2>جستجوی کالا</h2>
<input name="query" required placeholder="نام یا کد">
<button>جستجو</button>
</form>
</body></html>`);
});
...
app.post("/search",(req,res)=>{
  ...
  if(!matches.length){
    return res.send(`<h3>موردی یافت نشد</h3><a href="/">بازگشت</a>`);
  }
  ...
  let table = `<table border="1" style="width:100%;border-collapse:collapse;background:#fff">
<tr><th>نام</th><th>کد</th>`;
  keywords.forEach(k=> table+=`<th>${k}</th>`);
  table+=`<th>آپدیت</th></tr>`;
  ...
  for(const key in grouped){
    ...
    rows.forEach(r=>{
      keywords.forEach(k=>{
        if(r.item.includes(k)){
          lastText[k]=r.item.includes("استوک")
            ? `استوک - ${r.date}`
            : r.date;
          lastDate[k]=r.date;
        }
    ...
    table+=`<tr><td>${escapeHtml(name)}</td><td>${escapeHtml(code)}</td>`;
    keywords.forEach(k=>{
      let style="";
      if(lastDate[k]){
        const d=parseShamsiToDate(lastDate[k]);
        if(d){
          const diff=(today-d)/(1000*60*60*24);
          style=diff>365?"background:#cfc":"background:#fbb";
        }
      }
      table+=`<td style="${style}">${escapeHtml(lastText[k])}</td>`;
    });

    table+=`<td><button onclick="openUpdate('${escapeHtml(code)}','${escapeHtml(name)}')">به‌روزرسانی</button></td></tr>`;
  }
...
<div id="modal"><div id="box">
<h3>به‌روزرسانی</h3>
<input id="updateName" readonly style="width:100%"><br><br>
<select id="updateField">${keywords.map(k=>`<option>${k}</option>`).join("")}</select><br><br>
<select id="updateType"><option>نو</option><option>استوک</option></select><br><br>
...
function submitUpdate(){
 let field=updateField.value;
 let type=updateType.value;
 let date=updateDate.value.replace(/-/g,"/");
 fetch("/update",{method:"POST",headers:{"Content-Type":"application/json"},
 body:JSON.stringify({code:currentCode,name:updateName.value,
 item:type==="استوک"?"استوک "+field:field,date})})
 .then(r=>r.text()).then(()=>location.reload());
}

function downloadExcel(){
 fetch("/download-excel",{method:"POST",headers:{"Content-Type":"application/json"},
 body:JSON.stringify({query:"${escapeHtml(q)}"})})
 .then(r=>r.blob()).then(b=>{
  let a=document.createElement("a");
  a.href=URL.createObjectURL(b);
  a.download="SearchResults.xlsx";
  a.click();
 });
}
</script>
...
app.post("/update",(req,res)=>{
  const {code,name,item,date}=req.body;
  if(!item||!date) return res.status(400).send("ناقص");
...
app.post("/download-excel",(req,res)=>{
  ...
  const matched=filteredData.filter(r=>
    (r.code&&r.code.includes(q))||(r.name&&r.name.includes(q)));

  if(!matched.length) return res.status(204).send();
  ...
  for(const k in grouped){
    const g=grouped[k];
    const row={نام:g[0].name,کد:g[0].code};
    keywords.forEach(x=>row[x]="-");

    g.forEach(r=>{
      keywords.forEach(x=>{
        if(r.item.includes(x)){
          row[x]=r.item.includes("استوک")
            ? "استوک - "+r.date
            : r.date;
        }
      });
